...
public class LtSoHeadersController implements CodeMaster {
	...
	//ATFL Phase 2 new development 
		...
		@RequestMapping(value = "/getOrderForPendingApprovals", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE,headers = "X-API-Version=v1.0")
		public ResponseEntity<Status> getOrderForPendingApprovals(@RequestBody RequestDto requestDto) throws ServerException {
			try {
				ObjectMapper requestMapper = new ObjectMapper();
				String json = requestMapper.writeValueAsString(requestDto);
				String requestJson = requestMapper.writerWithDefaultPrettyPrinter().writeValueAsString(requestDto);
				...
				Status status = ltSoHeadersService.getOrderForPendingApprovals(requestDto);
